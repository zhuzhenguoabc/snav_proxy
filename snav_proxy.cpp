...
int main(int argc, char* argv[])
{
	...
	*/
	...
	float speed_coefficient = 1.0f;	//0.3f;	//0.5f; //speed limit coefficient
	//float height_limit = 20.0f;			//m

	float des_diff_limit = 1.2f;			//3.0f;
	float time_diff_limit = 0.06f;
	float over_speed_hover_time = 1.0f;		//1;
	float optic_flow_mission_time = 0.2f;	//1.0f;
	int  reverse_flag = 1;
	float  reverse_limit = 1;
	bool  over_speed_flag = false;
	static double t_over_speed_start = 0;
	static double t_optic_flow_mission_start = 0;
	static double t_reverse_or_hover_start = 0;
		...
		*/
		...
		else
		{
			...
			*/
			...
			{
				t_optic_flow_mission_start = t_des_now;
			}

			float est_sqrt = sqrtf(x_est*x_est + y_est*y_est);
			float des_sqrt = sqrtf(x_des*x_des + y_des*y_des);

			DEBUG("[%d] est_sqrt,des_sqrt,sqrt_diff=%f,%f,%f\n",
							loop_counter, est_sqrt, des_sqrt, (des_sqrt - est_sqrt));

			if ((t_des_now - t_optic_flow_mission_start > optic_flow_mission_time)
						...
						|| state == MissionState::LOITER))
			{
				...
				t_reverse_or_hover_start = t_des_now;

				DEBUG("[%d] over_speed_flag=true\n", loop_counter);
			}
			else
			{
			...
			/*
			if (fabs(des_sqrt - est_sqrt) > des_diff_limit)
			{
				DEBUG("[%d] over limit:t_des_now,t_over_speed_start,time_diff_limit=%lf,%lf,%f,%f\n",
							loop_counter, t_des_now, t_over_speed_start, (t_des_now - t_over_speed_start),time_diff_limit);

				if ((t_des_now - t_over_speed_start) > time_diff_limit)
				{
					over_speed_flag = true;
			...
			{
				t_over_speed_start = t_des_now;

				DEBUG("[%d] t_reverse_or_hover_start,t_des_now,diff=%lf,%lf,%f\n",
								loop_counter, t_reverse_or_hover_start,t_des_now,(t_des_now - t_reverse_or_hover_start));

				if ((t_des_now - t_reverse_or_hover_start) > over_speed_hover_time)
				{
					over_speed_flag = false;

					DEBUG("[%d] over_speed_flag=false\n", loop_counter);
				}
			}
			*/
			...
			*/
			...
			if (bReceivedUdpMsg)
			{
						...
						if (props_state == SN_PROPS_STATE_SPINNING)
						{
							...
							else
							{
								...
								else if (((/*z_est - z_est_startup*/revise_height) > 5.0f) && ((/*z_est - z_est_startup*/revise_height) <= 10.0f))
								{
									...
									{
										cmd0 = cmd0*0.7f*speed_coefficient;
										cmd1 = cmd1*0.7f*speed_coefficient;
									}
								}
								else if (((/*z_est - z_est_startup*/revise_height) > 10.0f) && ((/*z_est - z_est_startup*/revise_height) <= 15.0f))
								{
									...
									{
										cmd0 = cmd0*0.6f*speed_coefficient;
										cmd1 = cmd1*0.6f*speed_coefficient;
									}
								}
								else if (((/*z_est - z_est_startup*/revise_height) > 15.0f) && ((/*z_est - z_est_startup*/revise_height) <= 20.0f))
								{
							...
							if (over_speed_flag)
							{
								...
								{
									float roll = snav_data->attitude_estimate.roll;
									float pitch = snav_data->attitude_estimate.pitch;

									cmd0 = -pitch*reverse_limit;
									cmd1 = -roll*reverse_limit;
								}

								cmd2 = 0;
								cmd3 = 0;
								DEBUG("[%d] operation over_speed_flag cmd0,cmd1=%f,%f\n", loop_counter, cmd0, cmd1);
							}
				...
				else if ((gpsparams_udp.size() >= 2) && (gpsparams_udp[0].compare("des_limit") == 0))
				{
					des_diff_limit = atof(gpsparams_udp[1].c_str());
					DEBUG("udp receive  des_diff_limit=%f\n",des_diff_limit);
				}
				else if ((gpsparams_udp.size() >= 2) && (gpsparams_udp[0].compare("time_limit") == 0))
				{
					time_diff_limit = atof(gpsparams_udp[1].c_str());
					DEBUG("udp receive  time_diff_limit=%f\n",time_diff_limit);
				}
				else if ((gpsparams_udp.size() >= 2) && (gpsparams_udp[0].compare("gl_time") == 0))
				{
					optic_flow_mission_time = atof(gpsparams_udp[1].c_str());
					DEBUG("udp receive  optic_flow_mission_time=%f\n",optic_flow_mission_time);
				}
				else if ((gpsparams_udp.size() >= 2) && (gpsparams_udp[0].compare("hover_time") == 0))
				{
					over_speed_hover_time = atof(gpsparams_udp[1].c_str());
					DEBUG("udp receive  over_speed_hover_time=%f\n",over_speed_hover_time);
				}
				else if ((gpsparams_udp.size() >= 2) && (gpsparams_udp[0].compare("reverse_flag") == 0))
				{
					reverse_flag = atoi(gpsparams_udp[1].c_str());
					DEBUG("udp receive  reverse_flag=%d\n",reverse_flag);
				}
				else if ((gpsparams_udp.size() >= 2) && (gpsparams_udp[0].compare("reverse_limit") == 0))
				{
					reverse_limit = atof(gpsparams_udp[1].c_str());
					DEBUG("udp receive  reverse_limit=%f\n",reverse_limit);
				}
				...
				else if ((gpsparams_udp.size() >= 2) && (gpsparams_udp[0].compare("stop_flag") == 0))
				{
					...
					DEBUG("udp receive  optic_flow_error_stop_flag=%d\n",optic_flow_error_stop_flag);
				}
				*/
			}
			...
			if (over_speed_flag)
			{
				...
				cmd2 = 0;
				cmd3 = 0;
				DEBUG("[%d] mission over_speed_flag cmd0,cmd1=%f,%f\n", loop_counter, cmd0, cmd1);
			}
