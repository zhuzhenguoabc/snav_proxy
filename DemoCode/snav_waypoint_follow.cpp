...
int main(int argc, char* argv[])
{
 ...
 // Time after landing to stop props
  ...
  while (mission_in_progress)
  {
    ...
    if(mode == SN_OPTIC_FLOW_POS_HOLD_MODE){printf("\n[%d] SN_OPTIC_FLOW_POS_HOLD_MODE. ",loop_counter);}
    else if(mode == SN_GPS_POS_HOLD_MODE){printf("\n[%d] SN_GPS_POS_HOLD_MODE. ",loop_counter);}
    ...
    printf("[%d] waypoint_state: [%f,%f,%f,%f]\n",loop_counter,waypoints[current_wp].x, waypoints[current_wp].y, waypoints[current_wp].z, waypoints[current_wp].yaw);
    printf("[%d] output_vels: [%f,%f,%f,%f]\n",loop_counter,output_vel.x,output_vel.y,output_vel.z,output_vel.yaw);
    printf("[%d] position_est_startup: [%f,%f,%f]\n",loop_counter,x_est_startup,y_est_startup,z_est_startup);
    printf("[%d] distance_to_wp: [%f] lateral_speed [%f] \n",loop_counter,distance_to_wp,lateral_speed);
    printf("[%d] WP_goal_ret = [" BYTE_TO_BINARY_PATTERN "]\n", loop_counter, BYTE_TO_BINARY(wp_goal_ret&wp_goal_mask));
    ...
    {
      printf("[%d] WAYPOINT_FOLLOW: return: [%d]\n",loop_counter, wp_ret);
      printf("[%d] WAYPOINT #%u: [%f,%f,%f,%f]\n",loop_counter,
             current_wp,waypoints[current_wp].x,
             waypoints[current_wp].y, waypoints[current_wp].z, waypoints[current_wp].yaw);
    }
    else if (state == MissionState::LOITER)
      printf("[%d] LOITER\n",loop_counter);
    else if (state == MissionState::LANDING)
    ...
    if (isnan(cmd0)==1 || isnan(cmd1)==1 || isnan(cmd2)==1 || isnan(cmd3)==1){nan = 1;}
    if (nan == 1) {printf("Nan value occured");}
